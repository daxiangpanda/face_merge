...
def measure_triangle(image, points,src_points,dst_points):
    ...
    for p in points:
        # if(p[0] < 0 or p[1] < 0):
        #     return []
        # if(p[0] > image.shape[1] or p[0] > image.shape[0] or p[1] > image.shape[1] or p[1] > image.shape[0]):
        #     return []
        sub_div.insert(p)
    ...
    for t in triangle_list:
        ...
        pt2 = (t[2], t[3])
        pt3 = (t[4], t[5])
        d.line(list(pt1) + list(pt2) + list(pt3),width = 5)
        ...
        if len(ind) >= 3:
            # print(ind)
            triangle += list(itertools.combinations(ind,3))
        pt = []

    import os 
    if os.path.exists("tri_lines.jpg"):
        os.remove("tri_lines.jpg")
    if os.path.exists("tri_lines_dst.jpg"):
        os.remove("tri_lines_dst.jpg")
    if(os.path.exists("tri_lines.jpg")):
        pil_image.save("tri_lines_dst.jpg")
    else:
        pil_image.save("tri_lines.jpg")
    # Image.fromarray(np.uint8(image)).save("tri.jpg")
    # print(image.shape)
    # print(type(image))
    # print(len(points))
    return triangle
...
def morph_triangle(src, dst, img, t_src, t_dst, t, alpha,inde):
    ...
    # cv2.imwrite(str(inde) + "warp_img1"+".jpg",warp_img1)
